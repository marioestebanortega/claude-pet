...
def _default_epoch() -> int:
    """Fecha para los mtimes del paquete. Con `SOURCE_DATE_EPOCH` se respeta; si
    no, se usa la del último commit, para que el .deb sea reproducible por defecto
    (dos builds del mismo estado dan el mismo hash) en vez de depender del reloj."""
    ...
    try:
        ...
        out = subprocess.check_output(
            ["git", "-C", HERE, "log", "-1", "--format=%ct"],
            stderr=subprocess.DEVNULL)
        return int(out.strip())
    except Exception:
        return 1700000000   # constante estable si no hay git (sigue reproducible)
...
if __name__ == "__main__":
    ...
    print()
    print("Sin root (recomendado para un solo usuario):")
    print("    ./install-linux.sh --user")
    print()
    print("O con apt, para toda la máquina:")
